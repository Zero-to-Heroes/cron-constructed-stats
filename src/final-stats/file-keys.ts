...
const getDailyKeysForPreviousDays = (
	...
	while (firstDate < new Date()) {
		...
		keys.push(`${DECK_STATS_KEY_PREFIX}/decks/${format}/${rankBracket}/daily/${dateStr}.gz.json`);
	}
	return keys;
};
...
const getHourlyKeysForCurrentDay = (format: GameFormat, rankBracket: RankBracket): readonly string[] => {
